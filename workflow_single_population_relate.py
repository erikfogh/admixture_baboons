...
eastern_yellow_IDs = meta_data_samples.loc[meta_data_samples.C_origin == "Eastern Yellow, Tanzania"].PGDP_ID
...
keep_pop_l_l =[#["Ethiopian_Olive", etholive_IDs], #["Eastern_Yellow", eastern_yellow_IDs],
              # ["All_Samples", meta_data_samples.PGDP_ID], ["Tanzania_Olive", tanzolive_IDs],
                                                            ["Eastern_Yellow", eastern_yellow_IDs]]
...
vcfs = []
autosomes = list(range(1, 3)) #list(range(1, 21))
...
def full_relate(number, genetic_map, out_dir, pop_inf, prep_dir, base_path, relate_path):
    ...
    spec = """
    cd {}
    {} --mode All -m {} -N {} --haps {} --sample {} --map {} -o {} --annot {} --dist {} --memory 30
    """.format(out_dir,
